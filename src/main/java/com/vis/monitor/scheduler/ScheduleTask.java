...
public class ScheduleTask {
	...
	public void checkSystemMetrics() {
		...
		log.info(
				"CPU Usage: {}%, Total RAM: {} bytes, Free RAM: {} bytes, Total Memory: {} bytes, Used Memory: {} bytes",
		...
		diskInfo.setTotal(totalMemory);
		diskInfo.setUsed(usedMemory);
		
		sysInfo.setDiskInfo(diskInfo);
		
		MemoryInformation memInfo = new MemoryInformation();
		
		memInfo.setTotal(totalRAM);
		memInfo.setFree(freeRAM);
		
		sysInfo.setMemoryInfo(memInfo);

		SystemInformationService.addSystemInformation(sysInfo);
	}
	...
	private double getCpuUsage() {
		...
		// If the method is not available or the value is negative, return 0 (or another
		// appropriate value).
		return 0.0;
	}
	...
	private long getUsedMemory() {
		OperatingSystemMXBean osBean = ManagementFactory.getOperatingSystemMXBean();
		if (osBean instanceof com.sun.management.OperatingSystemMXBean) {
			long totalPhysicalMemorySize = ((com.sun.management.OperatingSystemMXBean) osBean)
					.getTotalPhysicalMemorySize();
			long freePhysicalMemorySize = ((com.sun.management.OperatingSystemMXBean) osBean)
		...
		// If the method is not available or the values are negative, return 0 (or
	...
	private void sendPortNotReachableEmail(User user, Server server) {
		...
		if (!isSent) {
			...
			try {
				...
				String message = "The IP address " + server.getHost() + " and port " + server.getPort()
						+ " are not reachable.";
